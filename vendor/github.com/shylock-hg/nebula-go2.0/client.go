...
func (client *GraphClient) Connect(username, password string) error {
	...
	if resp, err := client.graph.Authenticate([]byte(username), []byte(password)); err != nil {
		log.Printf("Authentication fails, ErrorCode: %v", resp.GetErrorCode())
		...
		log.Printf("Authentication fails, ErrorCode: %v", resp.GetErrorCode())
		return fmt.Errorf(resp.GetErrorCode().String())
	} else {
...
func (client *GraphClient) ExecuteJson(stmt string) ([]byte, error) {
